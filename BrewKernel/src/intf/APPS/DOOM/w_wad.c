...
wad_file_t *W_AddFile (char *filename)
{
    ...
    printf("[W_AddFile] Attempting to open: %s\n", filename);
    ...
    printf("[W_AddFile] Successfully opened %s, size=%u bytes\n", filename, wad_file->length);
    ...
    {
    	// single lump file
        printf("[W_AddFile] Treating %s as single lump file (not .wad)\n", filename);
    }
    else 
    {
    	// WAD file
        printf("[W_AddFile] Treating %s as WAD file (is .wad)\n", filename);
        W_Read(wad_file, 0, &header, sizeof(header));
        
        printf("[W_AddFile] WAD header raw bytes at offset 0: %02x %02x %02x %02x\n",
               ((unsigned char*)&header)[0],
               ((unsigned char*)&header)[1],
               ((unsigned char*)&header)[2],
               ((unsigned char*)&header)[3]);
        
        printf("[W_AddFile] WAD header: '%.4s', numlumps=%d, infotable_ofs=%d\n",
               header.identification, 
               *(int*)&header.numlumps,  // endian-raw value
               *(int*)&header.infotableofs);
		...
		printf("[W_AddFile] After byte-swap: numlumps=%d, infotable_ofs=%d\n",
        ...
        // Debug: show first lump directory entry raw bytes
        {
            unsigned char *p = (unsigned char*)fileinfo[0].name;
            printf("[W_AddFile] First lump dir entry raw bytes: %02x %02x %02x %02x %02x %02x %02x %02x\n",
                   (int)p[0], (int)p[1], (int)p[2], (int)p[3], (int)p[4], (int)p[5], (int)p[6], (int)p[7]);
        }
        
        newnumlumps += header.numlumps;
        printf("[W_AddFile] Added %d lumps, total lumps now: %d\n", header.numlumps, newnumlumps);
    }
    ...
    for (i=startlump; i<numlumps; ++i)
    {
		...
		// Debug: show first 2 lumps being added with raw bytes
		if (i < startlump + 2) {
		    unsigned char *p = (unsigned char*)lump_p->name;
		    printf("[W_AddFile] Lump %d (%s): %02x %02x %02x %02x %02x %02x %02x %02x (", i, filerover->name,
		           (int)p[0], (int)p[1], (int)p[2], (int)p[3], (int)p[4], (int)p[5], (int)p[6], (int)p[7]);
		    for (int b = 0; b < 8; b++) {
		        printf("%c", isprint(p[b]) ? p[b] : '.');
		    }
		    printf(")\n");
		}
...
int W_GetNumForName (char* name)
{
    ...
    if (i < 0)
    {
        printf("ERROR: W_GetNumForName: '%s' not found! (searched %d lumps)\n", name, numlumps);
        // Debug: print first 5 lumps with raw values
        printf("First 5 lumps:\n");
        for (int j = 0; j < 5 && j < numlumps; j++)
        {
            unsigned char *p = (unsigned char*)lumpinfo[j].name;
            printf("  Lump %d: %02x %02x %02x %02x %02x %02x %02x %02x\n", j,
                   (int)p[0], (int)p[1], (int)p[2], (int)p[3], (int)p[4], (int)p[5], (int)p[6], (int)p[7]);
        }
