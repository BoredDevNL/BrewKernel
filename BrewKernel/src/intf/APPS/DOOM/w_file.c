...
wad_file_t *W_OpenFile(char *path)
{
    ...
    // Always try to open from embedded memory first
    ...
    if (!M_CheckParm("-mmap"))
    {
        result = stdc_wad_file.OpenFile(path);
        if (result != NULL)
        {
    ...
    for (i = 0; i < arrlen(wad_file_classes); ++i)
    {
        ...
        if (result != NULL)
        {
            result->file_class = wad_file_classes[i];
            break;
        }
    }
